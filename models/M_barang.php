...
class M_barang extends CI_Model 
{
	function fetch_data_barang($like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL)
	{
		$sql = "
			SELECT 
				...
				CONCAT('Rp. ', REPLACE(FORMAT(a.`harga_jual`, 0),',','.') ) AS harga_jual,
				CONCAT('Rp. ', REPLACE(FORMAT(a.`harga_beli`, 0),',','.') ) AS harga_beli,
				a.`keterangan`,
				b.`kategori`,
		...
		if( ! empty($like_value))
		{
			...
			$sql .= "
				...
				OR CONCAT('Rp. ', REPLACE(FORMAT(a.`harga_jual`, 0),',','.') ) LIKE '%".$this->db->escape_like_str($like_value)."%' 
	...
	{
		return $this->db
			->select('id_barang, kode_barang, nama_barang, size, total_stok, harga_jual, harga_beli, id_kategori_barang, id_merk_barang, keterangan')
			->where('id_barang', $id_barang)
			->limit(1)
			->get('pj_barang');
	}

	function update_barang($id_barang, $kode_barang, $nama, $id_kategori_barang, $size, $id_merk_barang, $stok, $harga_jual, $harga_beli, $keterangan)
	{
		$dt = array(
