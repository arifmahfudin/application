		...
		if($level == 'admin' OR $level == 'inventory')
		{
			if($_POST)
			{
				...
				foreach($_POST['kode'] as $kode)
				{
					...
					$this->form_validation->set_rules('harga_jual['.$no.']','Harga Jual #'.($no + 1),'trim|required|numeric|min_length[4]|max_length[10]|callback_cek_titik[harga['.$no.']]');
					$this->form_validation->set_rules('harga_beli['.$no.']','Harga Beli #'.($no + 1),'trim|required|numeric|min_length[4]|max_length[10]|callback_cek_titik[harga['.$no.']]');
				...
				if($this->form_validation->run() == TRUE)
				{
					...
					foreach($_POST['kode'] as $k)
					{
						...
						$harga_jual			= $_POST['harga_jual'][$no_array];
						$harga_beli			= $_POST['harga_beli'][$no_array];
						$keterangan 		= $this->clean_tag_input($_POST['keterangan'][$no_array]);

						$insert = $this->m_barang->tambah_baru($kode, $nama, $id_kategori_barang,$size , $id_merk_barang, $stok, $harga_jual, $harga_beli, $keterangan);
						if($insert){
							$inserted++;
						}
						$no_array++;
			...
			if($level == 'admin' OR $level == 'inventory')
			{
				if($this->input->is_ajax_request())
				{
					...
					if($_POST)
					{
						...
						$this->form_validation->set_rules('harga_jual','Harga Jual','trim|required|numeric|min_length[4]|max_length[10]|callback_cek_titik[harga]');
						$this->form_validation->set_rules('harga_beli','Harga Beli','trim|required|numeric|min_length[4]|max_length[10]|callback_cek_titik[harga]');
						...
						if($this->form_validation->run() == TRUE)
						{
							...
							$update = $this->m_barang->update_barang($id_barang, $kode_barang, $nama,  $id_kategori_barang, $size, $id_merk_barang, $stok, $harga_jual, $harga_beli, $keterangan);
							if($update)
							{
								echo json_encode(array(
									'status' => 1,
